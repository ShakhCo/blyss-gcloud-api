...
/**
 * POST /cron/refresh-instagram-tokens
 * Refresh Instagram tokens expiring within 7 days
 */
router.post('/refresh-instagram-tokens', async (req, res) => {
    try {
        const { refreshLongLivedToken } = await import('../utils/instagram.js');

        const sevenDaysFromNow = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);

        const snapshot = await db.collectionGroup('instagram_connection')
            .where('token_expires_at', '<=', sevenDaysFromNow)
            .get();
        ...
        for (const doc of snapshot.docs) {
            try {
                const data = doc.data();
                const { access_token, expires_in } = await refreshLongLivedToken(data.access_token);
                const newExpiry = new Date(Date.now() + (expires_in - 5 * 24 * 60 * 60) * 1000);
                ...
                console.error(`[Cron] Failed to refresh Instagram token for ${doc.ref.path}:`, err.message);
                failedCount++;
            }
        }

        console.log(`[Cron] Instagram tokens refreshed: ${refreshedCount}, failed: ${failedCount}`);
        res.json({ refreshed: refreshedCount, failed: failedCount });
    } catch (error) {
        console.error('[Cron] Error refreshing Instagram tokens:', error);
        res.status(500).json({ error: 'Internal server error', error_code: 'INTERNAL_ERROR' });
    }
});
